...
export const ExploreView: React.FC<ExploreViewProps> = ({ activeDomain, setActiveDomain, onStartExploration }) => {
  return (
    <div className="px-6 space-y-12 flex flex-col h-full justify-center relative">
      {/* Background Glow */}
      ...
      <div className="text-center space-y-4 relative z-10">
        ...
        <h1 className="font-headline font-extrabold text-5xl text-on-surface tracking-tight leading-tight">
          掌握 <br/> <span className="text-gradient">未知</span>
        </h1>
        <p className="text-on-surface-variant font-medium text-lg max-w-[280px] mx-auto">
          选择一个领域开始你的神经扩展。
        </p>
      </div>

      <div className="flex flex-wrap justify-center gap-3 py-4 relative z-10">
        {CATEGORIES.map((cat) => (
          <button
            key={cat}
            onClick={() => setActiveDomain(cat)}
            className={cn(
              "px-6 py-3 rounded-2xl font-headline font-bold text-sm transition-all duration-300",
              activeDomain === cat 
      ...
      <div className="relative z-10 pt-8">
        <button 
          ...
          className="w-full max-w-md mx-auto py-5 primary-gradient text-white font-headline font-extrabold text-xl rounded-2xl shadow-[0_0_30px_rgba(79,70,229,0.3)] flex items-center justify-center gap-3 active:scale-[0.98] transition-all relative overflow-hidden group"
        >
